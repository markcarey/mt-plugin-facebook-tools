package MT::Auth::FacebookTools;
use strict;

use MT::Util qw( decode_url );
use FacebookTools::Util qw( get_facebook_client user_from_fb_profile );
...
sub login {
	...
	my $args = {};
	$args->{key} = 'FacebookTools';
	if ($q->param('code')) {
		($profile, $access_token) = FacebookTools::App::CMS::_do_oauth_login($app,'login_external',$args);
	} elsif ($q->param('oauth')) {
		return FacebookTools::App::CMS::_start_oauth_login($app,'login_external', $args);
	}
	
	return $app->error($app->translate("Could not verify the Facebook account specified")) if !$profile;
	
#	use Data::Dumper;
#	MT->log("FB profile is: " . Dumper($profile));
#	MT->log("FB profile link: " . $profile->{link});
#	MT->log("FB profile name: " . $profile->{name});
#	MT->log("FB profile id: " . $profile->{id});
	
	foreach my $bit (keys %{$profile}) {
#     MT->log("Key: $bit Value: $profile->{$bit}");
    }
...
sub load_auth {
    my $instance = MT->component('FacebookTools');
    return { 
        FacebookTools => {
            class => 'MT::Auth::FacebookTools',
            label => 'FacebookTools',
            logo  => 'plugins/FacebookTools/signin_facebook.png',
            logo_small => 'plugins/FacebookTools/facebook_logo.png',
            login_form => '
            	...
            	<input type="hidden" name="key" value="FacebookTools" />
            	<fieldset>
            	<div class="pkg">
            	  <p class="left">
            	    <input src="<mt:var name="static_uri">plugins/FacebookTools/images/signin_with_twitter.png" type="image" name="submit" value="<__trans phrase="Sign In">" />
            	  </p>
            	</div>
            	</fieldset>
            	</form>
            ...
            condition => sub {
                ...
                my $fb_api_id = $instance->get_config_value('facebook_application_id', "blog:" . $blog->id) || $instance->get_config_value('facebook_application_id', "system");
                my $fb_api_secret = $instance->get_config_value('facebook_application_secret', "blog:" . $blog->id) || $instance->get_config_value('facebook_application_secret', "system");
                unless ( $fb_api_secret && $fb_api_id ) {
                    $$reason = 
                        '<a href="?__mode=cfg_plugins&amp;blog_id=' . $blog->id . '">'
                        . $instance->translate('Set up Facebook Tools plugin')
                        . '</a>';
                    return 0;
                }
                return 1;
...
sub _callback_url {
    ...
    my $script;
#    if ($app->id eq 'community') {
#        $script = $app->config('CommunityScript');
#    } else {
